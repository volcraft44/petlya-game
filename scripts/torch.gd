...
func _process(delta):
	# На телефоне у факела нет света (он отключён ради FPS), поэтому весь
	# _process (мерцание света, проверка врагов, перерисовка пламени) —
	# бесполезная CPU-работа × десятки факелов. Полностью пропускаем:
	# пламя остаётся статичным, но кадры не тратятся.
	if _low_end:
		queue_redraw()   # один раз отрисовать статичное пламя
		set_process(false)
		return
	flicker_timer += delta
